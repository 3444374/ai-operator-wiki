...
## 相关概念
- [[concepts/ai-aware-query-optimization|AI 感知查询优化]] — 使优化器具备理解 ML 算子特性的能力，是本方法的关键使能技术。
- [[concepts/ai-函数作为-udf|AI 函数作为 UDF]] — 作为对比方案，ML-as-UDF 将 ML 函数外挂为 UDF，优化器无法穿透优化，凸显了原生集成的优势。
- [[concepts/写回瓶颈|写回瓶颈]] — 外部调用 ML 时常出现的数据持久化瓶颈，原生算子集成可避免该问题。
- [[concepts/分布式并行训练|分布式并行训练]] — 通过数据库分布式框架实现的高效训练方式，是原生集成带来的直接收益。

## 相关实体
- [[entities/gaussml|GaussML]] — 在 openGauss 中实践原生 SQL 算子集成的系统。
- [[entities/opengauss|openGauss]] — 为该方法提供数据库内执行环境的基础设施。
